...
const Page = () => {
  ...
  const { data: tasks, isLoading } = useSWR(
    authenticating ? null : "/api/user/today/tasks",
    fetcher
  );
  ...
  return (
    <AppLayout>
      <div className="mt-1 max-w-7xl mx-auto sm:px-6 lg:px-8 p-3">
        <div className="mt-1">
          {tasks.map((task) => (
            <div className="mt-1 p-1 bg-white rounded border" key={task.id}>
              <span className="text-purple-800 text-sm">
                {task.dayTitle} {task.arDate}
              </span>

              <div className="text-purple-800">{task.taskTitle} </div>
              ...
              {task.taskDoneAt && (
                <div className="text-green-800 text-sm">تمت: {task.taskDoneAt}</div>
              )}
